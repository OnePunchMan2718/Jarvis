...
try:
    ...
    print(
        "*For the successful execution of this program, it is essential to configure and set up the ChromeDriver."
    )
    print(e)
...
def ChatGPTBrain(Query):
    ...
    try:
        driver.find_element(
            by=By.XPATH,
            value="/html/body/div[1]/div/div/main/article/div/div/div/div/div/div/div[2]/div/div/div[2]/div/textarea",
        ).send_keys(Query)
        sleep(1)

    except:
        print(
            "*The Input button or Input Section does not appear to be readily accessible or available within the current context or environment.*"
        )
        print(
            "*Consider modifying the path for the 'Input' element, which is available on the website.*"
        )

    try:
        driver.find_element(
            by=By.XPATH,
            value="/html/body/div[1]/div/div/main/article/div/div/div/div/div/div/div[2]/div/div/div[2]/button/span",
        ).click()

    except:
        print(
            "*The Send.Keys() function or button does not appear to be readily accessible or available within the current context or environment.*"
        )
        print(
            "*Consider modifying the path for the 'Send' button, which is available on the website.*"
        )
    ...
    try:
        ...
        print(
            "*The response text element cannot be located. Please ensure that you update its selector or locator."
        )
